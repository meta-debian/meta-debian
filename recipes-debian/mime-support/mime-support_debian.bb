...
do_install () {
	...
	install -m 644 mailcap ${D}${nonarch_libdir}/mime/
	install -m 644 mailcap.order ${D}${sysconfdir}/
	install -m 755 update-mime ${D}${sbindir}/
	install -m 755 run-mailcap ${D}${bindir}/
	install -m 755 debian-view ${D}${nonarch_libdir}/mime/
	install -m 644 mailcap.entries ${D}${nonarch_libdir}/mime/packages/mime-support
