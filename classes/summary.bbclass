...
python do_package_write_deb_prepend() {
    import os.path
    import commands
    import re
...
def gen_summary(tree, csv):
    ...
    for line in lines:
        ...
        if len(line) is 0:
            ...
            # NOTE: exclude kernel module packages from summary because
            # do_package_write_deb_prepend() cannot embed metadata into
            # split packages generated by kernel-module-split.bbclass
            if not re.compile("^kernel-module-").match(info["Package"]):
